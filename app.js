...
function renderFeed() {
  document.querySelector('#feed').innerHTML = feedPosts.map((post, index) => `<article class="feed-post"><div class="feed-head"><div><span class="feed-name">${post.name}</span><span class="feed-time">${post.time}</span></div></div><p class="feed-text">${post.text}</p><div class="feed-actions"><button class="like-btn" data-index="${index}">♡ <span>${post.likes}</span></button><button class="share-btn" data-index="${index}">↗ แชร์</button><button class="reply-btn">↩ ตอบกลับ</button><span>•••</span></div></article>`).join('');
